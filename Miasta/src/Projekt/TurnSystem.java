...
public class TurnSystem  {   //Odpowiada za cały system tur
    ...
    public void simulationTurn(int i, int map_size, int number_cities) { //Jakbys sie zastanawial gdzie jest jedna petla to spiesze z pomoca classa TextWindow (Jebane okienka)
        ...
        for (int j = 0; j < number_cities; j++) {                       //Tura pojedynczego miasta
            ...
            if (TurnValue.checkCityUpgrade(citiesEvolution.getCity_table().get(j).getFortune(), citiesEvolution.getCity_table().get(j).getFields_number())) { //sprawdzenie czy może zlevelować
